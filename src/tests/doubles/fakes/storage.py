...
class FakeDropboxStorage(DropboxStorageInterface):
    """
    Fake Dropbox Storage class for unit testing.

    This class simulates a Dropbox storage by storing files in an internal dictionary
    instead of actually uploading them to a remote server.
    """
    ...
    async def upload_file(self, file_name: str, file_data: Union[bytes, bytearray]) -> None:
        """
        Simulates file upload to Dropbox by storing the file data in a dictionary.
    ...
    async def get_file_url(self, file_name: str) -> str:
        ...
        :return: The full fake URL to access the file.
        """
        return f"http://fake-dropbox.local/{file_name}"