...
from schemas import CartItemCreate, CartItemResponse, CartResponse, MessageResponseSchema
...
async def add_movie_to_cart(
    ...
    """Add movie to cart."""
    cart = await cart_crud.get_or_create_cart(db, current_user.id)
    cart_item = await cart_crud.add_movie_to_cart(db, cart.id, item.movie_id)

    if not cart_item:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Movie is already in cart or doesn't exist"
        )
